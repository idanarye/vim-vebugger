...
function! vebugger#lldb#start(binaryFile,args)
    ...
    let l:debugger=vebugger#std#startDebugger([l:debuggerExe, s:script_dir_path.'/lldb_wrapper.py', a:binaryFile])
...
function! vebugger#lldb#_readProgramOutput(line,readResult,debugger)
    if a:line=~'\v^program_stdout:' || a:line=~'\v^program_stderr:'
        let a:readResult.std.programOutput={'line':strpart(a:line, 16)}
    endif
endfunction

function! vebugger#lldb#_readWhere(line,readResult,debugger)
    if a:line=~'\v^where:'
...
function! vebugger#lldb#_readFinish(line,readResult,debugger)
    if a:line=~'\v^program_state:\sExited'
...
function! vebugger#lldb#_readEvaluatedExpressions(line,readResult,debugger) dict
