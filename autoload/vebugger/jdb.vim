function! vebugger#jdb#start(entryClass,args)
    let l:command = [vebugger#util#getToolFullPath('jdb',get(a:args,'version'),'jdb')]
    if has_key(a:args,'classpath')
        call extend(l:command, ['-classpath', a:args.classpath])
    endif
    let l:debugger=vebugger#std#startDebugger(l:command)
...
function! vebugger#jdb#_readProgramOutput(line,readResult,debugger) dict
    if a:line=~'\v^\> \>'
                \||a:line=='> '
...
function! vebugger#jdb#_readWhere(line,readResult,debugger)
...
function! vebugger#jdb#_readException(line,readResult,debugger)
...
function! vebugger#jdb#_readEvaluatedExpressions(line,readResult,debugger)
